...
class BotBAgent:
    """
    Bot B's model and recent conversation memory.
    ...
    def __init__(
        ...
        print(
            "[Bot B config] Ollama host: "
            "http://127.0.0.1:11434"
        )
        print(
            "[Bot B config] Proxy environment: ignored"
        )
    ...
    def _load_persona(self) -> str:
        if not self.persona_path.exists():
            raise FileNotFoundError(
                "Bot B persona file was not found:\n"
                f"{self.persona_path}"
            )
    ...
    def _remove_prefix(text: str) -> str:
        return re.sub(
            r"^\s*(drift|bot\s*b|assistant)\s*:\s*",
            "",
            text,
            flags=re.IGNORECASE,
        )
    ...
    @staticmethod
    def _is_meaningful_reply(text: str) -> bool:
        """
        Reject incomplete outputs such as:
        .
        ...
        -
        a single isolated character
        """
    ...
    def generate_reply(
        ...
        with self._generation_lock:
            ...
            print("[Bot B] generating...")

            reply = ""
            request_messages = list(messages)

            # MiniMax occasionally returns punctuation-only output.
            # Retry locally before adding anything to conversation history.
            for attempt in range(1, 4):
                ...
                if self._is_meaningful_reply(reply):
                    break

                print(
                    "[Bot B retry] incomplete output "
                    f"{reply!r}; retrying "
                    f"({attempt}/3)"
                )
                    ...
                    *messages,
                    ...
                    },
                    {
                        ...
                        "content": (
                            ...
                            "Reply again with one complete, natural "
                            "English sentence responding directly to "
                            "the previous message. Do not return only "
                            "punctuation or a fragment."
                        ),
                    },
                ]

            if not self._is_meaningful_reply(reply):
                raise RuntimeError(
                    "Bot B returned an incomplete response "
